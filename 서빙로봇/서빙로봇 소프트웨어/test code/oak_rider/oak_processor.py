...
class OakProcessor:
    ...
    def _setup_pipeline(self):
        ...
        cam_rgb.setPreviewSize(640, 400)  # 미리보기 크기 설정
    ...
    def get_frame(self) -> Optional[OakFrame]:
        ...
        # RGB 프레임 가져오기 (선택적)
        rgb = None
        in_rgb = self.q_rgb.tryGet()  # RGB 큐에서 데이터 가져오기
        if in_rgb is not None:
            rgb = in_rgb.getCvFrame()  # OpenCV 형식으로 변환
    ...
    def visualize(self, frame: OakFrame) -> np.ndarray:
        ...
        h, w = frame.depth_map.shape  # 깊이 맵 크기
        ...
        for obs in frame.obstacles:
            ...
            if obs.distance_m < config.ZONE_DANGER_M:
                color = (0, 0, 255)  # 빨강 (위험)
            cv2.rectangle(depth_color, (x1, y1), (x2, y2), color, 2)  # 바운딩 박스 그리기
        ...
        # 제목 표시
        cv2.putText(depth_color, "OAK-D-Lite", (6, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
        return depth_color  # 시각화된 이미지 반환
